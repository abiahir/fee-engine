...
class FeeEngineTest(unittest.TestCase):
    ...
    def test_zero_amount_is_accepted_not_rejected(self):
        """Boundary at exactly zero: the rule is "amount must be >= 0", so zero is valid.
 
        This test was added because mutation testing showed that changing the guard from
        `amount < 0` to `amount <= 0` killed no test: the boundary at zero was unpinned, so a
        defect rejecting free transactions would have shipped undetected.
        """
        result = self.engine_at(at(10)).price(self.card(amount="0.00"))
        self.assertEqual(Decimal("0.20"), result.fee)
